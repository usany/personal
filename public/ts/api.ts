...
export interface CaptureResult {
  /** Object URL for the returned image blob — assign to <img>.src, revoke when replaced. */
  objectUrl: string;
  blob: Blob;
  format: ScreenshotFormat;
  width: number;
  height: number;
  /** Server capture time in ms (from X-Screenshot-Duration-Ms), or null if absent. */
  durationMs: number | null;
  /** Filename from Content-Disposition, or a derived fallback. */
  filename: string;
}
...
export async function captureScreenshot(params: CaptureParams): Promise<CaptureResult> {
  ...
  const filename =
    filenameFromDisposition(h.get('Content-Disposition')) ??
...
async function parseError(res: Response): Promise<ScreenshotError> {
...
function normalizeFormat(value: string | null, fallback: ScreenshotFormat): ScreenshotFormat {
